...
def run_preprocess():
    ...
    hms = tf.expand_dims(tf.reduce_sum(hms, axis=-1), axis=-1)
    ...
    with tf.Session() as sess:
        ...
        [normed_dms_val, orig_dm_val, coms_val, pose_val, orig_pose_val, dc_val] = sess.run(
            [normed_dms, orig_dm, coms, norm_poses, orig_pose, hms])
